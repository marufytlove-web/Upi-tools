...
import { CopyIcon, Loader2Icon, RefreshCwIcon, SearchIcon, TicketIcon, WandSparklesIcon } from "lucide-react";
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[0.85fr_1.15fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Create a batch of recharge codes. Supported amounts are 1.8U, 5U, and 10U.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="space-y-1 text-sm font-medium">Amount
                <select value={amount} onChange={(event) => setAmount(event.target.value)} className="h-9 w-full rounded-lg border bg-background px-3 text-sm">
                  <option value="1.8">1.8 USDT</option>
                ...
                <tbody>
                  {filtered.length ? filtered.slice(0, 200).map((cdk) => (
                    <tr key={cdk.id} className="border-t">
                      ...
                      <td className="px-3 py-3"><Button variant="outline" size="sm" onClick={() => copyCode(cdk.code)}><CopyIcon />Copy</Button></td>
                    </tr>
                  )) : (
